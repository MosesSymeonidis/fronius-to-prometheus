...
var fronius = {
    ...
    getTemperatureChart: function (service, suffix, chartTitle, temperatureId, chartPriority) {
        ...
        var dim = {};
        dim[temperatureId] = this.createBasicDimension(temperatureId, "temperature", 1);
		chart = {
            id: chartId,                                         // the unique id of the chart
            name: "",                                            // the unique name of the chart
            title: service.name + " " + chartTitle,              // the title of the chart
            units: "C",                                          // the units of the chart dimensions
            family: "sensors",                                   // the family of the chart
            context: "fronius.temperature.output",               // the context of the chart
            priority: fronius.base_priority + chartPriority,     // the priority relative to others in the same family
            update_every: service.update_every,                  // the expected update frequency of the chart
            dimensions: dim
        };
        chart = service.chart(chartId, chart);
    ...
    getWattsPerSqm: function (service, suffix) {
        ...
        var dim = {};
        dim[fronius.wattsPerSqm] = this.createBasicDimension(fronius.wattsPerSqm, "wattsPerSqm", 1);
		chart = {
            id: chartId,                                         // the unique id of the chart
            name: "",                                            // the unique name of the chart
            title: service.name + " Watts per Square Meter",     // the title of the chart
            units: "W/m2",                                       // the units of the chart dimensions
            family: "sensors",                                   // the family of the chart
            context: "fronius.watts.per.sqm.output",             // the context of the chart
            priority: fronius.base_priority + 9,                 // the priority relative to others in the same family
            update_every: service.update_every,                  // the expected update frequency of the chart
            dimensions: dim
        };
        chart = service.chart(chartId, chart);
    ...
    getWindSpeed: function (service, suffix) {
        ...
        var dim = {};
        dim[fronius.windSpeed] = this.createBasicDimension(fronius.windSpeed, "windSpeed", 1);
		chart = {
            id: chartId,                                         // the unique id of the chart
            name: "",                                            // the unique name of the chart
            title: service.name + " Wind Speed",                 // the title of the chart
            units: "km/h",                                       // the units of the chart dimensions
            family: "sensors",                                   // the family of the chart
            context: "fronius.wind.speed.output",                // the context of the chart
            priority: fronius.base_priority + 10,                // the priority relative to others in the same family
            update_every: service.update_every,                  // the expected update frequency of the chart
            dimensions: dim
        };
        chart = service.chart(chartId, chart);
    ...
    processResponse: function (service, content) {
		...
		if (service.name === "GetPowerFlowRealtimeData") {
			chartDefinitions = fronius.parsePowerCharts(service, json);
		} else if (service.name === "GetSensorRealtimeData") {
			chartDefinitions = fronius.parseSensorCharts(service, json);
		}
    ...
    parsePowerCharts: function (service, json) {
    ...
    convertToJson: function (service, httpBody) {
        ...
        return this.isResponseValid(service, json) ? json : null;
    },

    // some basic validation
    isResponseValid: function (service, json) {
